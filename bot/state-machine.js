...
module.exports = function registerStateMachine(bot) {
  ...
  async function handleDeployFlow(bot, chatId, text, state, session) {
    ...
    if (state.step === "confirm") {
      ...
      const vmId = data.data?.id || "N/A";
      bot.sendMessage(chatId, MESSAGES.DEPLOY_SUCCESS(vmId), { parse_mode: "Markdown", ...REMOVE_KEYBOARD });
      // NFT confirmation (non-blocking)
      setTimeout(() => {
        bot.sendMessage(chatId,
          `🎨 *Deployment Proof NFT*\n\n` +
          `Your VM deployment has been recorded on-chain.\n` +
          `VM ID: \`${vmId}\`\n\n` +
          `Use /nft to view your collection.`,
          { parse_mode: "Markdown" }
        );
      }, 3000);
      sendMenu(chatId, session);
