...
export default function VMDashboard() {
  ...
  const handleStart = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        if (user) {
          await updateUserVm(user.userId, user.email, id, { state: "Running" });
        }
        await fetchVMs();
      } else {
  ...
  const handleStop = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        if (user) {
          await updateUserVm(user.userId, user.email, id, { state: "Stopped" });
        }
        await fetchVMs();
      } else {
  ...
  const handleDestroy = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        if (user) {
          await deleteUserVm(user.userId, id);
        }
        await fetchVMs();
      } else {
