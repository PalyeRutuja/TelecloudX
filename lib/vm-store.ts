...
// Lazy firestore import to avoid crashing during SSR/prerender when Firebase is not configured
let _firestore: Firestore | null = null;
function getFirestoreSafe(): Firestore {
  if (_firestore) return _firestore;
  try {
    const mod = require("@/lib/firebase");
    _firestore = mod.firestore ?? null;
  } catch {
    _firestore = null;
  }
  if (!_firestore || typeof (_firestore as any).collection !== "function") {
...
function vmCollection(userId: string) {
  return collection(getFirestoreSafe(), "users", userId, "vms");
}
...
export async function getUserVm(
  ...
  const snapshot = await getDoc(doc(getFirestoreSafe(), "users", userId, "vms", vmId));
...
export async function updateUserVm(
  ...
  const ref = doc(getFirestoreSafe(), "users", userId, "vms", vmId);
